...
const getUsersNft = async (req, res) => {
    ...
    const created = await Nft.find({author: _id}, {'imageUrl': 1, 'title': 1, 'author': 1, 'price': 1}).populate(
        'author',
        {'name': 1, 'avatarUrl': 1}
    );
    const owned = await Nft.find({owner: _id}, {'imageUrl': 1, 'title': 1, 'author': 1, 'price': 1}).populate(
        'author',
        {'name': 1, 'avatarUrl': 1}
    );
